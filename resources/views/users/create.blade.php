        ...
        <form action="{{ route('users.store') }}" method="POST" class="p-6">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-blue-50 border-l-4 border-blue-500 p-4 rounded">
                    <label class="block text-sm font-medium text-gray-700 mb-3">
                        <i class="fas fa-school text-blue-600 mr-1"></i>
                        École de rattachement
                    </label>
                    <p class="text-xs text-gray-500 mb-3">L'utilisateur ne verra que les données de son école. Laisser vide pour un accès à toutes les écoles (admin global).</p>
                    <select name="school_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        <option value="">— Toutes les écoles —</option>
                        @foreach($schools as $school)
                        <option value="{{ $school->id }}" {{ old('school_id') == $school->id ? 'selected' : '' }}>{{ $school->name }} ({{ $school->type_label }})</option>
                        @endforeach
                    </select>
                </div>
                @endif
