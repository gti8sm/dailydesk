...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white shadow rounded-lg p-4 {{ $comment->is_staff ? 'border-l-4 border-blue-500' : 'border-l-4 border-gray-300' }}">
            <div class="flex items-center justify-between mb-2">
                <div class="flex items-center gap-2">
                    ...
                    @if($comment->is_staff)
                    <span class="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">Support</span>
                    @endif
                </div>
    ...
    <div class="bg-white shadow-lg rounded-xl p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">Ajouter un message</h3>
        <form action="{{ route('support.comment', $ticket) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-4">
                <textarea name="content" rows="4" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent" placeholder="Votre message..."></textarea>
            </div>
            <div class="mb-4">
    ...
    <div class="bg-gray-50 rounded-lg p-4 text-center text-gray-500">
        ...
        <form action="{{ route('support.comment', $ticket) }}" method="POST" class="mt-3">
            @csrf
            <input type="hidden" name="content" value="Ticket rouvert par le demandeur">
